...
export async function fetchGraphData(props) {
    let { dataGetter, comparison_pair, codeset_ids, cids, newCset = {} } = props;
    ...
    if (comparison_pair) {
        comparison_rpt = dataGetter.axiosCall(
            `single-n3c-comparison-rpt?pair=${comparison_pair}`,
...
export function CsetComparisonPage() {
    ...
    if (editingCset) {
        infoPanels.push(
            <FlexibleContainer key="cset" title="New concept set" startHidden={false}
                               hideShowPrefix={true}
            ...
            conditionalCellStyles: [
                {
                    ...
                    style: (row) => ({
                        padding: '0px 3px 0px ' +
                            (nested ? (9 + row.depth * 19 + (row.hasChildren ? 0 : 8)) : 19) +
                            'px',
                    }),
                },
            ],
...
function downloadCSV(props, tsv = false) {
    ...
    const cset_keys = codeset_ids.map(
        id => selected_csets.find(cset => cset.codeset_id === id).concept_set_name);
    /*
    ...
    return (
        <>
            <ol>
                ...
                <LI>
                    When you get to the screen for editing the new draft version, you will
                    see
                ...
                <LI>
                    Paste the JSON copied into your clipboard from VS-Hub earlier into the
                    box, and
