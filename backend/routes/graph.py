...
async def concept_graph(request: Request, codeset_ids: List[int] = Query(...),
                        cids: Optional[List[int]] = Query(None)) -> List:
    cids = cids if cids else []
    return await concept_graph_post(
        request=request, codeset_ids=codeset_ids, cids=cids)
...
async def concept_graph_post(
    ...
    try:
        ...
        # nodes_in_graph, missing_in_between_nodes, preferred_concept_ids, orphans_not_in_graph, hidden_nodes, hidden_dict = \
        #     get_connected_subgraph(REL_GRAPH, codeset_ids, cids, hide_vocabs=['RxNorm Extension'])
        ...
        concept_metadata, hidden_dict, nonstandard_concepts_hidden = filter_concepts(_concept_metadata, hide_vocabs)
        concept_ids = set([c['concept_id'] for c in concept_metadata])
        # todo: Not using cids. make a note about that?
        ...
        missing_in_betweens = ''
        concept_metadata_m, hidden_dict_m, nonstandard_concepts_hidden_m = filter_concepts(missing_in_betweens, hide_vocabs)
...
def filter_concepts(
    concepts: List[Dict[str, Tuple[int, str, str]]],
    hide_vocabs: List[str],
    nonstandard_concepts_hidden=False
) -> Tuple[Set, Dict, Set]:
